...
Tolle mentioned outlier related issues corresponding to humidity measurements where %RH was greater than 100%. 

We observed a similar problem in our dataset based on out analysis. Visually inspecting the histogram and corresponding boxplot we see a number of readings over 100%, clearly indicating the same problem. Measurements corresponding to these readings were excluded from analysis.
...
Visually inspecting temperature plots, we see a large number of outliers in the boxplot. A quick google search for hottest recorded temperature in Sonoma, CA is 44 degrees Celsius. Tolle's measurement ranges for temperature indicate a max value of 32.6 degrees Celsius. Looking at a temperature vs time plot at the height level it becomes clear that temperatures recorded in the high 30s/low 40s are erros. Thus, all measurements greater than 32.6 were removed.
...
# try and facet wrap charts
# all %>% 
#   select(humidity, humid_temp, incident_par) %>%
#   gather() %>%
#   ggplot(aes(value)) + 
#     facet_wrap(~ key, scales = "free") +
#     geom_boxplot() +
...
boxplot(all$humidity, main = "Humidity - Original ")
all = all %>% filter(humidity <= 100)
boxplot(all$humidity, main = "Humidity - Outliers Removed")

# remove measurements based on temperature cutoff
boxplot(all$humid_temp, main = "Temperature - Original ")
all = all %>% filter(humid_temp <= 32.6)
boxplot(all$humid_temp, main = "Temperature - Outliers Removed")
