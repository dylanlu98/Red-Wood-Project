...
ggplot(all, aes(x = epoch, fill = data_source)) +
  ...
  # cosmetics
  labs(x="Epoch", y="Frequency", fill="Data Source") +
...
ggplot(all, aes(voltage, fill = data_source)) +
  ...
  # cosmetics
  labs(x="Voltage", y="Frequency", fill="Data Source") +
  scale_fill_brewer(palette="Blues", labels=c("Logger", "Network"))
```
...
Hamatop and hamabot measures had consistent units across data sources, but did not match the scale reported in the Tolle paper. These units were converted to match with the following formula.

$$ \text{Incident PAR} = [hamatop \ or \ hamabot]*0.0185 $$
...
all %>% 
  ...
  ggplot(aes(x=data_source, y=n, fill = data_source)) +
    ...
    # cosmetics
    labs(x="Data Source", y="Record Count") +
...
The time of day for the missing measurements appear uniform across the 24-hour day, however there is a large spike at XXX. Over the course of the experiment, we see more measurement issues than at the beginning which echos the sentiment expressed in the Tolle report.
...
Tolle mentioned outlier related issues corresponding to humidity measurements where %humidity was greater than 100%. 

We observed a similar problem in our dataset. Visually inspecting the histogram and corresponding boxplot we see a number of readings over 100% threshold, which were excluded from analysis.

Visually inspecting temperature plots, we see a large number of outliers. A quick google search for the hottest recorded temperature in Sonoma, CA revealed 44 degrees Celsius, much lower than the 100+ degree points indicated on the plots. Tolle's max temperature reading was 32.6 degrees and because we are unfamilar with the climate in the geographic region we decided to use this as our cutoff filter. All measurements greater than 32.6 were removed.

The incident par histogram shows a long tail distribution. In the boxplot we see two distinct groups of outliers separated by almost 1000 units. We determined to remove the second group by enforcing a manual cutoff of < 2500 because it seemed to represent sensor failure due to the low number of points.
...
boxplot(all$humid_temp, main = "Temperature - Original", ylab = "\u00B0C")
all = all %>% filter(humid_temp <= 32.6)
boxplot(all$humid_temp, main = "Temperature - Outliers Removed", ylab = "\u00B0C")

# remove incident par outliers
boxplot(all$incident_par, main = "Incident PAR - Original", ylab = expression(mu~ "mol/m"^"2"~"/s"))
all = all %>% filter(incident_par < 2500)
boxplot(all$incident_par, main = "Incident PAR - Outliers Removed", ylab = expression(mu~ "mol/m"^"2"~"/s"))
par(mfrow = c(1,1))
```


### (e bonus - discuss other possible outliers and explain reason why it is better to remove them than to keep)
As we noted above, we found a large number of duplicate entries from both measurement sources. These duplicated entries were removed to prevent overweighting distributions. The incident PAR boxplot indicated a large number of potential outliers even after initial removal. However we decided to include them in anaylsis because we felt like the nighttime 0 PAR readings were skewing the quantiles closer towards 0. We also are not familiar with the flux units on the dimension and did not feel that we possessed eough expertise to make a judgement call one way or the other.

NaN issue (all rows are blank).
...
```{r, screeplot, fig.cap = "PCA Scree Plot Explaining Variability"}
...
PCA was performed on the following dimensions: humidity, temperature, incident PAR, reflected PAR, and height. The Scree Plot shows the first three components explain almost 90% of the variability in the data and indicates that a lower dimensional representation is possible.

```{r, lower-dim-representation}
# project points onto PCAs
# this should be used as interesting finding #1
...
Here the first two principal component score vectors were plotted for each of the height classes. What's interesting is we see tight groupings for sensors mounted lower on the tree and much more variability between measurements taken higher up. We were initially suspecting the variability was due to the amount of measurements taken at each height, however looking at the table below we see the most measurements in the 40-50 meter height range, but much more variability in the 60+ meter class. Intuitively we suspect that the top of the tree presents a harsher environment (more exposed to the elements) than the base. Which could explain why we see such a wide dispersion of points at the highest points compared to the bottom.
...
# this just gives me a close up of the max seen in the chart...
plot(mx_reflected_par_pts$date_concat, mx_reflected_par_pts$reflected_par)

# histogram of the reflected par max readings
dtTime = as.numeric(mx_reflected_par_pts$date_concat - trunc(mx_reflected_par_pts$date_concat, "days"))
hist(dtTime, title = "Time of Day - Max Reflected PAR Readings")
...
# can we cluster measurements by sensor?
...
### Interesting question 3 - DL
Does the height determine the humidity and temperature during day time?
```{r, interesting-finding-3 - DL}
...
Height_numeric = as.numeric(Height_character)
# temperature
...
ggarrange(temp, humid,
                    labels = c("Temperature", "Humidity", "Incident PAR", "Reflected PAR"),
                    ncol = 1, nrow = 2)
```

Our hypothesis is that the temperature and humidity measured should be lower at the bottom of the red wood during day time. The explanation is that the top of the read wood receive a larger amount of incident PAR and reflective PAR, and therefore have more energy and higher temperature. Correspondingly, The higher the temperature, the faster the water vapors, which leads to a lower humidity. However, The stacked box plot based on height during day time (6am-8pm) shows that the humidity and temperature do not vary significantly as height increases during the day time. Our findings correspond to the findings of Tolle (Tolle,2005). The stacked box plot from Tolle shows that the red wood has a consistent humidity and temperature through out the day and night. In conclusion, we suppose that red woods have a spacial homeostasis mechanism that regulates its temperature and humidity.It would be an interesting research topic to find out the working principle of this homeostasis.
...
```{r,q5b-height-value-box-plots-DL}
# try from 6:00 to 20:00
day_time = all %>%
  filter(time >= "06" & time <= "20")
