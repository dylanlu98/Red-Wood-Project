...
```{r, identify-outliers}
vars_want = all[c("humidity","humid_temp","incident_par","reflected_par")]
...
quantile(all$humid_temp, prob= c(0.01, 0.99))
quantile(all$humidity, prob= c(0.01, 0.99))
quantile(all$incident_par, prob= c(0.01, 0.99))
quantile(all$reflected_par, prob= c(0.01, 0.99))
```
...
all %>%
  filter(time >= "06" & time <= "12")

ggplot(all %>% filter(time >= "06" & time <= "12"), aes(x=humid_temp, y=humidity) ) +
  geom_point() +
  ggtitle("Humidity vs. Temperature - 6:00 to 12:00")

ggplot(all %>% filter(time >= "06" & time <= "18"), aes(x=incident_par, y=humidity) ) +
  geom_point() +
  ggtitle("Humidity vs. Incident PAR - 6:00 to 18:00")
