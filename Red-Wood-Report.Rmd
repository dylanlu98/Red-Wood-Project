...
boxplot(all$incident_par, main = "Inci. PAR - Original", ylab = expression(mu~ "mol/m"^"2"~"/s"))
all = all %>% filter(incident_par < 2500)
boxplot(all$incident_par, main = "Inci. PAR - Outliers Removed", ylab = expression(mu~ "mol/m"^"2"~"/s"))
par(mfrow = c(1,1))
```

### (e - discuss other possible outliers and explain)
...
```{r, smoosh-time-plots-work,  fig.height = 7, fig.width = 7, fig.align="center"}
...
ggplot(mx_reflected_par_pts, aes(date_concat, reflected_par, color = timeboundary)) + 
  geom_point() + 
...
#                    ncol = 1, nrow = 2)
```
![Box plot of humidity and temperature along heights](./q4-interesting finding 3-height-value-box-plots.jpeg)
...
>>>>>>> 24b760e04661b27d0178c0044350c99a20ed82a2
...
With the data stratified by day/night the gradients over the height of the tree become much more clear. For example, on the incident...
...
### Any suggestions for Tolle Figure 4?
