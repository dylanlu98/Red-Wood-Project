...
```{r, 5a-incident-charts, fig.show="hold", out.width="45%"}
# incident PAR charts
...
ggplot(all, aes(x = incident_par)) +
  ...
  labs(title = 'Incident PAR', subtitle = 'Original Scale (No Log Transform)') +
  xlab(expression('μmol/m'^2*'/s'))

# Log Transform
ggplot(all, aes(x = log(incident_par))) +
  geom_histogram(aes(y = (..count..)/sum(..count..)), binwidth = .1) +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(title = 'Incident PAR', subtitle = 'Log Scale') +
  xlab(expression('ln(μmol/m'^2*'/s)'))

```
We see the log chart more accurately convey the long tail, however, it might be misleading at first glance as PAR measurements appear more concentrated. It's important that the reader understand the log nature of the chart before reaching an incorrect conclusion and is why we've decided to display charts side-by-side.

```{r, 5a-reflected-charts, fig.show="hold", out.width="50%"}
# reflected PAR
